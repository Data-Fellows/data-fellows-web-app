export const foundingTeam = [
  {
    name: "Tobi Oladimeji",
    role: "CEO & Founder",
    tagline: "Dreams up the next wave of data tools for small businesses.",
    image: "/images/TOBI OLADIMEJI-FOUNDER.jpg",
    linkedin: "https://www.linkedin.com/in/tobioladimeji/",
  },
  {
    name: "Oyindamola Victor",
    role: "CTO",
    tagline: "Builds smarter product systems with the community.",
    image: "/images/Oyindamola_Victor_cto.JPG",
    linkedin: "https://www.linkedin.com/in/oyindamolavictor/",
  },
  {
    name: "Elijah Soladoye",
    role: "Founding Software Engineer",
    tagline: "Builds scalable and efficient systems for the community.",
    image: "/images/Elijah_Founding_Software_Engineer.jpg",
    linkedin: "https://www.linkedin.com/in/elijah-soladoye/",
    objectPosition: "center 50%",
  },
  {
    name: "Sami Ansari",
    role: "Founding AI Engineer",
    tagline: "Turns complex models into tools people can actually use.",
    image: "/images/Sami_Founding_AI_ENGINEER.png",
    linkedin: "https://www.linkedin.com/in/samiansari99/",
  },
];

export const communityAdmins = [
  {
    name: "Sambo Evelyn",
    role: "Community & Growth Operations Manager",
    tagline: "Keeps programs coordinated and member growth loops healthy.",
    image: "/images/growth_manager.jpg",
    featured: true,
    linkedin: "https://www.linkedin.com/in/sambo-evelyn",
    objectPosition: "center 25%",
  },
];

export const advisors = [
  {
    name: "Nnamdi Oboli",
    role: "Co-Founder Dioni Visions Limited",
    tagline: "Supports partnerships and market expansion in Nigeria, bringing experience from building and scaling consumer-facing ventures and understanding how local markets adopt and trust new products.",
    image: "/images/Mr_Nnamdi Oboli-Co-Founder.jpg",
    linkedin: "https://www.linkedin.com/in/nnamdi-o-247a991a7/",
  },
  {
    name: "Oyeta Kokoroko",
    role: "Founder, Okoko Cosmetiques",
    tagline:
      "Guides Inscend's growth with insights from building one of Canada's leading clean beauty brands.",
    image: "/images/advisor.webp",
    linkedin: "https://www.linkedin.com/in/oy%C3%A9ta-kokoroko-9b52b6160",
  },
];
