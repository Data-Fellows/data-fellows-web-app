...
// Dummy fetchPayments for demo; replace with your actual API call
          ...
          payments.map((payment) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                {payment.match.applicant.photoUrl ? (
                  <img
                    src={payment.match.applicant.photoUrl}
                    alt="Applicant"
                    className="w-8 h-8 rounded-full object-cover border border-border"
                    loading="lazy"
                  />
                ) : (
                  <span className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-muted-foreground text-xs font-bold">
                    N/A
                  </span>
                )}
                <span className="truncate font-medium">
                  {payment.match.applicant.firstName || "N/A"}{" "}
                  {payment.match.applicant.lastName || ""}
                </span>
              </div>
              ...
              <a
                href={`/employer/payments/${payment._id}`}
      ...
      <div className="hidden sm:block overflow-x-auto rounded-xl border border-border shadow-lg bg-card">
        <div className="min-w-[700px]">
          <div className="grid grid-cols-6 bg-muted/70 text-foreground font-semibold text-sm">
            <div className="py-3 px-2">Applicant</div>
